...
plan-reviewer agent を並列起動する。各 reviewer にStep 1で取得したPlan出力先ディレクトリ、ユーザーの元の要求、現在のラウンド番号を渡す。
...
plan-creator を再起動し、Plan出力先ディレクトリ、ユーザーの元の要求、adopted / partial の指摘内容を渡す。
