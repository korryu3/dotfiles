...
- 🔴 **[must-fix]** [`code-review`] `auth/login.ts:42` — JWTの有効期限がハードコードされている
- 🟡 **[should-fix]** [`nitpicker`] `api/users.ts:15-20` — ページネーション未実装
- 🟢 **[nit]** [`nitpicker`, `security-review`] `models/order.ts:88` — 既存クエリがソフトデリートを考慮していない
```
